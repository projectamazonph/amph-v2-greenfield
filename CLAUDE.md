...
**Current addendum, 2026-09-24:** `main` is reviewed at `2a7e8bdd`. PRs #612–#616 merged 2026-09-24: dropped early-bird pricing (both the feature and the `seed-pricing-tiers` early-bird dry-run leak), self-hosted four font families via `@fontsource`, and wired the glossary popover into the lesson renderer. `docs/admin-backend.md` is being rewritten against the current codebase (PR #617). `docs/audit-2026-09-23-lesson-arithmetic.md` is the numeric record. One thing no deploy does for you: curriculum text reaches learners only after someone runs `node scripts/seed-all-content.mjs` against the intended database, so merged lesson fixes stay invisible in production until then (`content/README.md` has the table).
