...
- **Completeness findings from the 2026-07-27 audit:** `PrismaBadgeRepository.create/update/archive` are still stubs even though the production container wires that adapter; the four graded simulator actions use `userId: "system"`; the admin seed script constructs `PrismaClient` directly instead of using the Prisma 7 driver adapter; first-time impersonation does not preserve the admin token; and `GetAdminDashboardStats.pendingRefunds` is hardcoded to zero. Fix or explicitly gate these paths before calling the corresponding product features complete. (The quiz-lesson placeholder from that audit is fixed — QUIZ lessons now link to the quiz player, and `/courses/[slug]/quizzes/[quizId]` gives the course's knowledge checks a real entry point.)
- **`Course.curriculum` is a denormalized read model and it is load-bearing.** The catalog page reads the relational `Module`/`Lesson` tables, but the lesson page, `AuthorizeLessonAccess` and `MarkLessonComplete` all resolve lessons against the `Course.curriculum` JSON. Anything that writes modules or lessons MUST run `RebuildCourseCurriculum` afterwards, or the catalog will link to lessons that 404. Both importers had this bug until 2026-07-29. Every admin mutation use case already does it.
- **`LessonSidebar` takes a plain view model, not the `Course` entity.** It is a client component, and `Course.price` is a `Money` class instance that React cannot serialize across the server/client boundary. Passing the entity in 500s the page.
- **`Lesson.type` is the authoritative discriminator, not `content.type`.** The importers write the content blob as bare `{ body }` / `{ durationMinutes }` with no type key. Renderers must key off the column.
