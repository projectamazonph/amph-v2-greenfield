...
**Current addendum, 2026-09-25:** `main` is reviewed at `2af741a5`. PRs #612–#618 merged across 2026-09-24 and 2026-09-25: dropped early-bird pricing (both the feature and the `seed-pricing-tiers` early-bird dry-run leak), self-hosted four font families via `@fontsource`, wired the glossary popover into the lesson renderer, fully rewrote `docs/admin-backend.md` against the code, and added a sibling `docSymbolAndRouteReferences.test.ts` that catches URL-shaped route claims the path guard structurally cannot see. Two PRs blocked on Ryan (curriculum teaching decisions, `Last verified` dates for the 29 fact-card lessons). `docs/audit-2026-09-23-lesson-arithmetic.md` is the numeric record. One thing no deploy does for you: curriculum text reaches learners only after someone runs `node scripts/seed-all-content.mjs` against the intended database, so merged lesson fixes stay invisible in production until then (`content/README.md` has the table).
