...
**Current addendum, 2026-09-23:** `main` is reviewed at `b2e7fe3`. The line of work is content quality: #581 through #595 plus #597 merged on 2026-09-23, correcting lesson figures and answer keys, the quiz bank's voice, and the MDX directive plugin that rendered a literal `:::` under all four trade-off blocks. The verified gate on `main`, read out of the CI jobs rather than a local run: 5,208 Vitest tests passed with 3 skipped across 538 collected files, 892 architecture checks, TypeScript clean, ESLint 0 errors and 3 pre-existing warnings, and the production build, Playwright E2E, learning-release gate and Lighthouse jobs all green. `STATE.md` is the live record and `docs/audit-2026-09-23-lesson-arithmetic.md` is the numeric one; read both before relying on the older addenda below. One thing no deploy does for you: curriculum text reaches learners only after someone runs `node scripts/seed-all-content.mjs` against the intended database, so merged lesson fixes stay invisible in production until then (`content/README.md` has the table).
