...
**Addendum, 2026-08-04 (STORY-085, scenario publishing + versioning — full rewire):** `SimulatorScenario` now has a real `draft → published → archived` lifecycle with version history (`scenarioKey`/`version`/`status` fields, `PublishSimulatorScenario`/`CreateScenarioVersionDraft`/`ListScenarioVersions` use cases, admin UI at `/admin/simulators` + `/admin/simulators/[id]/versions`). All 5 simulator practice pages (`src/app/tools/<name>/page.tsx`) now fetch `container.scenarioRepo.findPublished(simulatorId)` server-side instead of importing a hardcoded `SCENARIO` const — publishing a new version through the admin UI actually changes what students see and get graded against. Each simulator's server action also now resolves its scenario server-side rather than trusting client-echoed data, closing a real trust gap in bid-elevator, str-triage, and listing-audit (all three used to accept the full scenario — economics, lexicons, category/niche — back from the client on submit). campaign-builder and bid-elevator's practice pages were switched from a legacy preview-only action (`buildCampaign()`, `runBidElevator()` — now deleted) to their existing but previously-unwired graded, persisted-attempt use cases (`campaignBuilderAttempt()`, `bidElevatorAttempt()`), so those two simulators now create a real `SimulatorAttempt` record for the first time. **Two real limitations surfaced, not fixed:** neither campaign-builder nor listing-audit has a UI for the free-form submission their richest grading path expects (`userAdjustedCampaigns` / `userFindingActions` respectively) — building either editor is a separate feature, not a rewire. listing-audit's page still calls the preview-only `auditListing()` for this reason; campaign-builder's persisted attempts still have `scoreDimensions`/`feedback` stay `null` without that editor. See `docs/stories/STORY-085.md` for the full 6-stage breakdown.

**Addendum, 2026-08-03 (STORY-095.5, email templates wired into the send path):** the STORY-095 caveat below ("editing a template here does not yet change what Resend actually sends") is **closed**. All 7 `*Email.tsx` renderers now accept `headlineOverride`/`introBodyOverride`/`ctaLabelOverride` (a shared `EmailTemplateOverride` type in `src/ports/email/EmailTemplateOverride.ts`), and every triggering call site (`ResendVerification`, `VerifyEmail`, `RequestPasswordReset`, `IssueCertificate`, `SendLiveClassReminders`, the shared `sendRefundEmail()` in `ProcessRefund.ts`, and the PayMongo webhook's `sendReceiptEmail()`) fetches `emailTemplateRepo.findByType(...)` and falls back to the original hardcoded copy when no admin row exists. **Two real limitations, not bugs:** `EmailTemplate` has no `{{placeholder}}` syntax, so a customized field replaces the default verbatim and loses any per-recipient interpolation (e.g. the student's first name) that default used to have; and `RefundEmail` has no CTA button at all, so its `ctaLabel` field is accepted for interface consistency but never renders anywhere (a pre-existing model/renderer mismatch, not introduced or fixed here). See `docs/stories/STORY-095.5.md`.
