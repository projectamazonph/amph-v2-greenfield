...
**Current addendum, 2026-09-25:** `main` is reviewed at `4f3688d0`. The in-scope series from #612 through #621 is fully closed. PRs #612–#616 dropped early-bird pricing, fixed the vercel deploy tiers, self-hosted four fonts via `@fontsource`, and wired the glossary popover into the lesson renderer. PRs #617–#619 rewrote `docs/admin-backend.md` against the code, added a sibling `docSymbolAndRouteReferences.test.ts` that catches URL-shaped route claims the path guard structurally cannot see, and updated the records. PRs #620–#621 applied all 8 of Ryan's dashboard decisions: 7 curriculum teaching decisions (broad 40%/exact 25%; product targeting cap 5-10%; all bid-multiplier times PHT; negate at 5+ clicks; CVR-CI framing qualified as worst-case at 50% CVR; ACoS 0% replaced with N/A on zero-sales terms; "The 30-Minute Weekly Review" renamed to "The Weekly Check-in") and the single `Last verified: 2026-09-25` date on all 29 fact-card lessons. `docs/audit-2026-09-23-lesson-arithmetic.md` is the numeric record. Open follow-ups: run `node scripts/seed-all-content.mjs` to make the curriculum fixes visible to learners, set `Next review due:` dates on the 16 fact cards that still carry the placeholder, and address the `9.3` ladder boundary overlap as a separate fix.

**Addendum, 2026-08-12:** `main` was reviewed at `ee1737a`. PR #305 repaired the student journey and accessibility states. PR #306 fixed manually granted students by creating eligible published-course enrollments instead of changing only `subscriptionTier`. PR #307 fixed the admin-login redirect cookie. PR #308 fixed forgot-password links by normalizing the retired deployment origin to `https://projectamazonph.vercel.app`. The gate measured then was 3,816 Vitest passed, 2 skipped, and 665 architecture checks.
...
**Addendum, 2026-09-23 (curriculum inventory and lesson-to-tool bridges never reach the UI):** `content/curriculum/inventory.json` declares a `toolBridge` for all 45 lessons, and five point at a simulator (`2.2` keyword-research, `3.1` listing-audit, `4.4` campaign-builder, `6.3` bid-elevator, `7.3` str-triage). `src/domain/curriculum/CurriculumInventory.ts` parses it and `scripts/validate-tool-bridges.ts` gates it on every release, but no file under `src/app`, `src/components`, `src/composition` or `src/infra` references `toolBridge`, `CurriculumInventory` or `inventory.json`. Lesson MDX also contains no markdown link or `href` anywhere, so a lesson that tells a learner to open the Search Term Triage tool gives them no clickable way to do it. Treat `inventory.json` as validator input, not a render source. Same shape as the glossary entry below, and `docs/audit-2026-09-23-lesson-arithmetic.md` carries the full record.

**Addendum, 2026-09-23 (STORY-131 glossary popover was never wired):** `content/curriculum/glossary.json`, `src/lib/glossary.ts`, and `src/components/lesson/GlossaryTerm.tsx` all exist and the lib has unit tests, but the lesson renderer never mounts the popover. `GlossaryTermButton` has no importer outside its own module, `loadGlossaryManifest` and `lookupGlossaryTerm` are called only from `src/lib/__tests__/glossary.test.ts`, and the `data-amph-term` hook the story says the directive plugin emits exists nowhere in the code. `CHANGELOG.md:32` and the `#522` row in `STATE.md` report the feature as shipped. Learners read the inline parenthetical the author wrote, which is the documented no-JavaScript fallback, so this is dead code plus a stale doc claim, not a broken lesson. Do not assume a glossary lookup surface exists: there is no `/glossary` route, the manifest is not a search index, and `lookupGlossaryTerm` matches an explicit slug only. Finishing it needs a term directive in the content pipeline and an a11y pass, so it is an open decision, not an incidental fix.
...
As of 2026-07-27 the items below were the current verified gaps. The audit that produced them, `docs/audit-2026-07-27-completeness-review.md`, was removed on 2026-09-14 by `e1f7352` ("consolidate documentation, remove 60+ stale/superseded files", PR #513), so this section, `STATE.md` and `SESSION-HANDOVER.md` are what survives of it. Several claims that used to live in this section — `src/lib/`/`src/components/` not existing, no admin panel, `courseRepo`/`orderRepo` on in-memory repos, the PayMongo webhook bypassing the container, only one migration, DB "not provisioned", `Course.curriculum` drifting from `Module`/`Lesson`, `SignUp.ts` not writing an audit entry, no persistent webhook event log, no admin 2FA, no admin-specific login route, no runbooks, and a missing admin seed script — are no longer true. Re-verify against the source before trusting historical notes.
