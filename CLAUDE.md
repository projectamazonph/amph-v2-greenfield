...
- `package.json`'s `db:seed:admin` script points at `scripts/seed-admin-user.mjs`, which does not exist in `scripts/` (only `gen-secret.js`, `import-amph-content.ts`, `seed-pricing-tiers.ts`, `seed-simulator-policies.ts` are present). Don't assume that script works — see `docs/runbooks/admin-access-recovery.md` for the SQL-based alternative.
