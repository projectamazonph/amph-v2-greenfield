...
### 2026-07-26: Simulator scoring rebuilds — Sprint 13 closeout

- The four simulator rebuilds (STORY-067/068/069/070) and the supporting attempt-infrastructure / scoring-engine / feedback-composer work (STORY-064/065/066) all landed against the same `ScorePolicy` dimensional model. The model itself ships with the attempt infrastructure; `scorePolicyRepo` is wired into the production container (PR #172).
- **PR #189** (`e322ece`): `feat(simulator): STORY-070 Listing Audit rebuild — scoring engine integration`. `ListingAuditOutput` now carries `ScoreDimensions` (`titleCoverage`, `bulletCoverage`, `imageCoverage`, `backendKeywords`, `priorityCoverage`); `userDecisions` on the input side records the learner's per-finding "fix"/"skip" choice so `run()` can grade against the dimension model. Listing-audit legacy `auditListing()` preserved for backward compat.
- **PR #180** (`90014bb`): `feat(simulator): STORY-068 Bid Elevator rebuild — scoring engine integration`. `BidElevatorOutput` now carries `ScoreDimensions` (`bidAccuracy`, `budgetAdherence`, `roasHit`, `explanation`); `BidRecommendation` carries `groundTruth` / `userBid` / `isCorrect`. 49 Bid Elevator tests pass (25 simulator + 13 action + 11 page).
- **PR #179** (`9eb5f6b`): `feat(simulator): STORY-067 STR Triage rebuild — scoring engine integration`. `StrTriageOutput` now carries `ScoreDimensions` (`direction`, `profitability`, `dataSufficiency`, `explanation`); per-classification `groundTruth` / `userChoice` / `isCorrect`. 41 STR Triage tests pass.
- **PR #182** (`a0ce6c2`): `fix(auth): plant session cookie on the redirect response (STORY-066 follow-up)`. The hotfix that made the signup E2E that had been failing across every PR since #169 finally pass. Root cause: `/api/auth/{login,signup,logout}` route handlers were calling `setAuthCookie` via `cookies().set()` (writes to the implicit response) and then returning `NextResponse.redirect()` — a fresh response that did not inherit the cookie. Three-part fix: `setAuthCookie` / `clearAuthCookie` now accept an optional `CookieTarget`; `performSignUp` / `performLogin` now expose `sessionToken` / `expiresAt`; the `Secure` flag and cookie name (`amph_session` vs `__Secure-amph_session`) are now both derived from the request protocol (`isHttps`), not `NODE_ENV`. 2827 tests pass.
- **PR #181** (closed as redundant): the Campaign Builder rebuild (STORY-069) code landed via the PR #182 squash from a diverged local main. The work itself is on `main`; the PR was redundant against the squash. `seed-simulator-policies.ts` was fixed at the same time: campaign-builder policies now use the correct dimension names.
- Full per-PR detail (intermediate PRs #166–#170, #172, #173, including the move of `login` + `signup` to API routes, the React 19 `useActionState` crash fix in #163, and the rate-limit reconciliation #145 that had to come first) lives in `SESSION-HANDOVER.md`'s "What changed this session" entries for 2026-07-24 → 2026-07-25.
- **Scope caveat carried forward to Sprints 14–16.** The audit at `docs/audit-2026-07-26-simulator-accuracy-review.md` (commissioned in parallel) verified that the rebuilds land the scoring infrastructure, not the scoring _content_: the four simulators still return a hardcoded `explanation: 100` while policies weight that dimension 10–25%, Listing Audit still passes on a "fix"-everything click pattern at every difficulty, and the four seeded policies sum to 0.90 (capping a flawless learner at 90) because `seed-simulator-policies.ts` writes via raw `prisma.upsert` and `PrismaScorePolicyRepository` reads via `hydrateScorePolicy()` which skips the `isValidPolicy()` guard. `CLAUDE.md` warns against building certification or job-readiness signals on current simulator scores; remediation is sequenced as Sprints 14–16 (STORY-071–089).

### 2026-07-24: Test expectations sync (rename + simulator count)

- **PR #158** (merged as `0243b55`): `fix: sync test expectations with rename + simulator count changes`
