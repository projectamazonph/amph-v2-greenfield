...
This PR ships four coordinated fixes:
...
- `.github/workflows/ci.yml`: the e2e job now runs `pnpm db:seed:scenarios` between `prisma generate` and `pnpm build` so the test database has the same published scenario rows that `vercel.json` adds on the production deploy path. Without it, the new `simulator-access.spec.ts` regression test fails on every viewport (the page errors before hydration, so the Run simulation button never mounts), and any other e2e test that touches `/tools/<simulator>` hits the identical `scenario_not_found` boundary that surfaced on production before this PR. Idempotent upsert on `id` means re-running across CI reruns is safe.
