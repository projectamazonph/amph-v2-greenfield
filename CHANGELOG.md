...
Fix: the seed mints a real TOTP secret with production otpauth params (`OtpauthTotpService`: 20-byte secret, base32, SHA1, 6 digits, 30s period) into `twoFactorSecret`, and journeys 3, 4, and 5 submit the current code in the form's Two-factor code field, exercising the real 2FA login path with no product-code bypass. URL assertions tightened to `/\/admin$/` so any future bounce fails fast at login. New `tests/unit/e2e-helpers/seedAdminTotp.test.ts` pins helper to production-verifier interop without a database. Full CI green including E2E.

### 2026-09-07: UI overlap, padding, responsiveness, and back-navigation fixes (PR #478)

Full UI audit of 80 page routes plus layouts, shells, theme, and component CSS. Overlap and layering: student drawer raised above the mobile toggle, ScrollToTop lowered below the nav overlay, Toast and CommandPalette moved onto the token z-index scale with mobile side margins, viewport meta gained device-width and initial scale, global `table` rule reverted from block display (it broke column layout, scroll moved to a wrapper class), admin table scroll enabled on all widths, hero badge and lesson-arrow overhang contained, impersonation banner made non-sticky so it no longer collides with sticky headers, pricing highlight switched from 2px border to outline (no 1px grid shift), mobile dropdown given a scroll bound. Padding and fluidity: dialog actions wrap with reduced padding under 480px, simulator forms reduce padding on narrow screens, ad-console frame uses dvh sizing with a mobile floor, footer architecture table gained a scroll wrapper, fixed-height covers use aspect ratio, bid input and breadcrumb truncation use fluid clamps, 4-across grids collapse to one column under 480px, tools links meet the 44px touch target. Navigation: `/pricing` wrapped in `StudentShell` like `/faq`, reset-password token page and verify-email sent views gained sign-in and request-new-link escapes, three missing `admin/resources` loading skeletons added, `RouteError` gained dashboard and courses links. The lesson page keeps its focus layout (breadcrumb plus Back to Course) by design.

CI caught one self-inflicted a11y regression (the new footer scroll region was not keyboard-focusable), fixed with `role="region"` plus `tabindex` on the footer architecture table, and one brittle caption pin relaxed for prettier wrapping (`round30-table-scope-caption.test.ts`). A parallel commit on the same branch (dashboard card inset consolidation, bid table fixed layout) is included in the squash. Verification: tsc clean, lint 0 errors, unit plus integration pass, build and Lighthouse pass.
