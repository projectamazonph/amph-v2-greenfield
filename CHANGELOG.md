...
### 2026-07-29: Student-facing lessons, quizzes and simulators made functional

Verified against a real Postgres 16 with the actual seed data and a real browser, not only unit tests.

- **Every seeded lesson page returned a 404.** Both importers (`ImportAmphContent` and `scripts/seed-all-content.mjs`) created courses with a placeholder `Course.curriculum`, wrote the real content as `Module`/`Lesson` rows, and never rebuilt the denormalized curriculum JSON. The catalog page reads the relational tables so it listed all 31 lessons, but the lesson page, `AuthorizeLessonAccess` and `MarkLessonComplete` all resolve against `Course.curriculum`, so every lesson link off the catalog dead-ended. Both importers now run the existing `RebuildCourseCurriculum` step. Measured before and after against a real database: 31 of 31 lesson links went from 404 to 200.
- **Fixing that exposed a 500 on every readable lesson.** `LessonSidebar` is a client component and was handed the whole `Course` entity, whose `price` is a `Money` class instance, which React cannot serialize across the server/client boundary. It now takes a plain view model, which also keeps the lesson `content` blob out of the client bundle. The crash had been masked by the 404.
- **Lessons then rendered with no body.** `LessonContent` dispatched only on a `type` discriminator inside the content blob, but the importers write `{ body }` / `{ durationMinutes }` and treat the `Lesson.type` column as authoritative, so every seeded lesson fell through to "Lesson content unavailable". Dispatch now prefers the column and falls back to the blob for older rows.
- **Lesson completion was unreachable.** The `MarkLessonComplete` use case has existed since STORY-027 but was absent from the composition container and had no server action and no UI, so enrollment progress sat at 0% forever, no lesson XP was awarded, and no certificate could ever become eligible. Added `progressEventRepo` and `markLessonComplete` to both containers, a `markLessonComplete` server action, and a complete button on the lesson page. The lesson page now loads real `completedLessonIds` from the enrollment instead of a hardcoded empty array.
- **The Bid Elevator produced no visible output.** `BidElevatorForm` stashed the simulator result in browser storage and refreshed the route, but the page is a server component that never read it back and never rendered `BidElevatorResult` at all, so "Run simulation" appeared to do nothing. The result now lives in component state and renders below the form, matching the other four simulator forms.
- **Quizzes had no entry point.** A `Quiz` carries a `courseId` and no lesson or module key, but the only quiz route looked one up by lesson id, which no seeded content satisfies. Added `/courses/[slug]/quizzes/[quizId]` (course-scoped, enrollment-gated, admins exempt), listed each course's knowledge checks on the course page, and replaced the "coming soon" card on QUIZ lessons with a real link. The lesson-scoped route now points at the course page instead of dead-ending.
- **Sidebar section headers were inert.** They rendered as buttons with `aria-expanded`, but the open state was a const, so clicking did nothing and the ARIA state misreported to screen readers. Now real state.
- **The dashboard "Sign out" link 404'd.** It pointed at `/logout`, which is not a route; logout is POST-only on purpose, since a GET logout is a CSRF hole. Replaced with the same POST form the admin `UserCard` already uses.
- **`InMemoryEnrollmentRepository.create` froze the stored enrollment**, diverging from its own `update()` and from `PrismaEnrollmentRepository`. `Enrollment` is a mutate-then-persist entity, so any test that enrolled a student and then completed a lesson threw. Aligned the fake with the real adapter.
- Also fixed a latent foreign-key hazard in `seed-all-content.mjs`, which recomputed course ids from a hash instead of reading them back from the upsert, so modules would attach to a nonexistent course id whenever a course already existed under a different id.
- Coverage: 3,205 unit and integration tests pass (up from 3,172), including new suites for the completion action, the container wiring end to end, the quiz route, the Bid Elevator result panel, and the importer content shapes. Typecheck, lint, 539 architecture tests, and the production build are all clean.
