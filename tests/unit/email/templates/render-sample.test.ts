 ...
 * Run with: SAMPLE_OUTPUT_DIR=/some/path npx vitest run tests/unit/email/templates/render-sample.test.ts
 *
 * Writes sample-receipt.html, sample-cert.html, etc. to SAMPLE_OUTPUT_DIR
 * (defaults to /workspace) for visual inspection. Not part of the regular
 * suite — skipped in CI by the absence of SAMPLE_OUTPUT_DIR.
 */
...
import { WelcomeEmail } from "@/infra/email/templates/WelcomeEmail";
import { PasswordChangedEmail } from "@/infra/email/templates/PasswordChangedEmail";
import { PaymentFailedEmail } from "@/infra/email/templates/PaymentFailedEmail";

const SAMPLE_OUT = process.env.SAMPLE_OUTPUT_DIR;

describe("writes sample emails for visual inspection (skipped unless SAMPLE_OUTPUT_DIR is set)", () => {
  it.skipIf(!SAMPLE_OUT)("renders each email template to SAMPLE_OUTPUT_DIR", () => {
    const out = SAMPLE_OUT!;
    const samples = [
      {
        name: "sample-receipt",
      ...
      },
      {
        ...
        html: renderToStaticMarkup(
          RefundEmail({
            ...
            dashboardUrl: "https://amph.example.com/dashboard",
          }),
        ),
      },
      {
      ...
      {
        name: "sample-password-changed",
        html: renderToStaticMarkup(
          PasswordChangedEmail({
            firstName: "Maria",
            changedAt: new Date("2026-08-01T15:00:00Z"),
          }),
        ),
      },
      {
        name: "sample-payment-failed",
        html: renderToStaticMarkup(
          PaymentFailedEmail({
            firstName: "Maria",
            courseTitle: "PPC Deep Dive",
            retryUrl: "https://amph.example.com/checkout?courseSlug=ppc-deep-dive",
          }),
        ),
      },
    ];
    for (const s of samples) {
      writeFileSync(
        `${out}/${s.name}.html`,
        `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body>${s.html}</body></html>`,
      );
    }
  });
});
