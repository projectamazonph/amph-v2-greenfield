...
export function AnnotatedListingCanvas({
  id,
  title,
  sections,
  prompt,
      ...
      <div className={styles.listingLayout}>
        <div className={styles.listingMock} aria-label="Annotated listing preview">
          ...
          <div className={styles.productCopy}>
            ...
            <strong>
              {sections.find((section) => section.id === "title")?.content ?? "Product title"}
            </strong>
            <span className={styles.starLine}>★★★★☆ · 4.6 · 500 reviews</span>
            <ul>
              {sections
                .filter((section) => section.id.startsWith("bullet"))
                .slice(0, 3)
                .map((section) => (
        ...
        <div className={styles.annotationPanel}>
            ...
            {sections.map((section) => {
              ...
              return (
                <button
                  ...
                  aria-controls={`${id}-annotation-${section.id}`}
                  onClick={() => setActiveId(section.id)}
                  className={`${styles.annotationTab} ${selected ? styles.annotationTabActive : ""}`}
                >
                  {section.label}
                </button>
              );
          ...
          {active ? (
            ...
            >
              ...
              <h3>{active.label}</h3>
              <p>
                <strong>Current pattern:</strong> {active.content}
              </p>
              <p className={styles.effectLine}>
                <strong>PPC job:</strong> {active.effect}
              </p>
            </div>
          ) : (
  ...
  return (
    <li className={styles.hierarchyNode}>
      <details open={depth < 2}>
        <summary>
          <span className={`${styles.nodeBadge} ${styles[`node-${node.type}`]}`}>
            {node.type.replace("-", " ")}
          </span>
  ...
  return (
    <section className={styles.block} id={id} aria-labelledby={`${id}-title`}>
      ...
      <div className={styles.hierarchyLegend} aria-label="Hierarchy levels">
        <span>
          <i className={`${styles.nodeBadge} ${styles["node-account"]}`} /> Account
        </span>
        <span>
          <i className={`${styles.nodeBadge} ${styles["node-campaign"]}`} /> Campaign
        </span>
        <span>
          <i className={`${styles.nodeBadge} ${styles["node-ad-group"]}`} /> Ad group
        </span>
        <span>
          <i className={`${styles.nodeBadge} ${styles["node-target"]}`} /> Target
        </span>
      </div>
      ...
      {note ? (
        <p className={styles.note}>
          <strong>Structure cue:</strong> {note}
        </p>
      ) : null}
    </section>
  );
        ...
        {stages.map((stage, index) => {
          ...
          return (
            <button
              ...
              role="tab"
              aria-selected={selected}
              aria-controls={`${id}-stage-${stage.id}`}
              onClick={() => setActiveId(stage.id)}
              className={`${styles.funnelStage} ${selected ? styles.funnelStageActive : ""}`}
            >
              <span className={styles.funnelStageNumber}>{index + 1}</span>
        ...
        >
          <div>
            <p className={styles.panelLabel}>
              Stage {stages.findIndex((stage) => stage.id === active.id) + 1}
            </p>
            <h3>{active.question}</h3>
          </div>
          <div>
            <span className={styles.panelLabel}>Formats that fit</span>
...
export function CompetitiveGapMatrix({
  id,
  title,
  dimensions,
  competitors,
  ...
  return (
    <section className={styles.block} id={id} aria-labelledby={`${id}-title`}>
      <div className={styles.blockHeader}>
        ...
        {revealMode === "after-choice" ? (
          <button
            ...
            onClick={() => setShowCoach((visible) => !visible)}
            aria-expanded={showCoach}
          >
            {showCoach ? "Hide coach rationale" : "Reveal coach rationale"}
          </button>
        ) : null}
      </div>
      <div
        className={styles.competitiveTableWrap}
        tabIndex={0}
        role="region"
        ...
        <table className={styles.competitiveTable}>
          ...
          <tbody>
            {competitors.map((competitor) => (
              <tr
                key={competitor.id}
                className={competitor.id === active?.id ? styles.competitorActive : ""}
              >
                <th scope="row">
                  <button
                    type="button"
                    className={styles.competitorButton}
                    aria-pressed={competitor.id === active?.id}
                ...
                {dimensions.map((dimension, index) => (
                  <td key={`${competitor.id}-${dimension}`}>
                    {competitor.values[index] ?? "Not observed"}
                  </td>
                ))}
              </tr>
            ))}
      ...
      {active ? (
        <div className={styles.competitiveReadout} role="status">
          <div>
          ...
          {showCoach ? (
            <div>
              <span className={styles.panelLabel}>Coach rationale</span>
              <p>{active.signal}</p>
              <p className={styles.actionLine}>{active.action}</p>
            </div>
          ) : (
            <div>
              <span className={styles.panelLabel}>Your move</span>
              <p>
                State whether this competitor creates an opportunity, vulnerability, or contested
...
export function InsightRouter({ id, title, routes, revealMode = "always" }: InsightRouterProps) {
  ...
  return (
    <section className={styles.block} id={id} aria-labelledby={`${id}-title`}>
      ...
      <div className={styles.insightRoutes} role="tablist" aria-label="Competitive insight routes">
        {routes.map((route, index) => (
          <button
            ...
            type="button"
            role="tab"
            aria-selected={route.id === active?.id}
            aria-controls={`${id}-route-${route.id}`}
            onClick={() => setActiveId(route.id)}
            className={`${styles.insightRouteButton} ${route.id === active?.id ? styles.insightRouteActive : ""}`}
          >
            <span>{index + 1}</span>
            {route.signal}
          </button>
      ...
      {active ? (
        <div
          id={`${id}-route-${active.id}`}
          className={styles.insightDetail}
          role="tabpanel"
          tabIndex={0}
          aria-live="polite"
        >
          {showCoach ? (
            <>
              <div>
                <span className={styles.panelLabel}>Implication</span>
                <p>{active.implication}</p>
              </div>
              <div>
                <span className={styles.panelLabel}>Evidence next</span>
                <p>{active.evidence}</p>
              </div>
              <div>
              ...
              {active.owner ? <span className={styles.ownerTag}>Owner: {active.owner}</span> : null}
            </>
          ) : (
            <div>
              <span className={styles.panelLabel}>Your move</span>
              <p>
                Choose the next evidence check and bounded action before revealing the coach
                rationale.
              </p>
            </div>
          )}
...
export function TimelineCalendar({
  ...
  const highlightedRows = useMemo(
    () => rows.filter((row) => row.values[activePeriod]),
    [rows, activePeriod],
  );
  ...
  return (
    <section className={styles.block} id={id} aria-labelledby={`${id}-title`}>
      ...
      <div className={styles.timelineToolbar}>
        <span>
          Inspect period: <strong>{periodLabel}</strong>
        </span>
        <div className={styles.periodButtons} role="tablist" aria-label="Timeline periods">
          {periods.map((period, index) => (
            <button
              ...
              type="button"
              role="tab"
              aria-selected={index === activePeriod}
              onClick={() => setActivePeriod(index)}
              className={index === activePeriod ? styles.periodActive : ""}
            >
              {period}
            </button>
          ))}
        ...
        <table className={styles.timelineTable}>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id}>
                <th scope="row">
                  <span className={`${styles.toneBar} ${styles[`tone-${row.tone ?? "accent"}`]}`} />
                  {row.label}
                ...
                {periods.map((period, index) => (
                  <td
                    key={`${row.id}-${period}`}
                    className={index === activePeriod ? styles.periodCellActive : ""}
                  >
                    {row.values[index] ?? ""}
                  </td>
                ))}
      ...
      <div className={styles.periodReadout} role="status">
        <strong>{periodLabel}</strong>
        <span>
          {highlightedRows.map((row) => `${row.label}: ${row.values[activePeriod]}`).join(" · ")}
        </span>
      </div>
      {note ? (
        <p className={styles.note}>
          <strong>Planning cue:</strong> {note}
        </p>
      ) : null}
    </section>
  );
