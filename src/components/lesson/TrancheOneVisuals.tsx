    ...
    <section className={styles.block} id={id} aria-labelledby={`${id}-title`}>
            ...
            <tr>
              ...
              {columns.map((column, index) => (
                <th
                  key={column}
                  scope="col"
                  className={highlightedColumn === index ? styles.highlightedHeader : undefined}
                >
                  {column}
                </th>
              ))}
              ...
              <tr key={row.label} className={row.emphasis ? styles[row.emphasis] : undefined}>
                ...
                {columns.map((column, index) => (
                  <td
                    ...
                    className={highlightedColumn === index ? styles.highlightedCell : undefined}
                  >
      ...
      {note ? (
        <p className={styles.note}>
          <strong>Decision cue:</strong> {note}
        </p>
      ) : null}
    </section>
  );
    ...
    <section className={styles.block} id={id} aria-labelledby={`${id}-title`}>
      ...
      <div className={styles.classificationToolbar}>
        ...
        <button
          ...
          onClick={() => setShowRationales((visible) => !visible)}
          ...
          {steps.map((step, index) => {
            ...
            return (
              <button
                ...
                className={`${styles.flowButton} ${selected ? styles.flowButtonActive : ""}`}
              >
                <span className={styles.stepNumber}>{index + 1}</span>
        ...
        {active ? (
          ...
          >
            ...
            {showCoach ? (
              <>
                <p>
                  <strong>Evidence to inspect:</strong> {active.evidence}
                </p>
                <p className={styles.actionLine}>
                  <strong>Decision boundary:</strong> {active.action}
                </p>
              </>
            ) : (
              <p className={styles.actionLine}>
                <strong>Your move:</strong> State the evidence you would inspect and the smallest
                safe action before revealing the coach rationale.
              </p>
            )}
          </div>
        ) : (
...
export function SimulationRubric({
  id,
  title,
  scenario,
  criteria,
  ...
  return (
    <section className={styles.block} id={id} aria-labelledby={`${id}-title`}>
      ...
      <div className={styles.scenarioBox}>
        ...
        {submission ? (
          <p className={styles.submission}>
            <strong>Submit:</strong> {submission}
          </p>
        ) : null}
      </div>
      <div className={styles.rubricHeader}>
        <div>
          <strong>
            {complete} of {criteria.length} criteria reviewed
          </strong>
          <div
