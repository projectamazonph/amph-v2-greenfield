...
function VisualShell({
  ...
  return (
    <section
      ref={ref}
      id={id}
      className={`${styles.visual} ${revealed ? styles.revealed : ""}`}
...
function DiagnosticMap({ id, title }: { id: string; title: string }) {
  const stages = [
    ...
    {
      label: "Sale",
      detail: "Did the economics work?",
      metric: "ACoS / ROAS / TACoS",
      tone: "purple",
    },
  ];
  return (
    <VisualShell id={id} title={title} eyebrow="Diagnostic map">
      <div className={styles.mapLegend} aria-label="Metric groups">
        <span>
          <i className={`${styles.legendDot} ${styles.orange}`} /> Traffic
        </span>
        <span>
          <i className={`${styles.legendDot} ${styles.green}`} /> Conversion
        </span>
        <span>
          <i className={`${styles.legendDot} ${styles.purple}`} /> Economics
        </span>
      </div>
      ...
      <p className={styles.visualTakeaway}>
        <strong>Operating rule:</strong> locate the broken handoff before choosing the lever.
      </p>
    </VisualShell>
  );
}
    ...
    <VisualShell id={id} title={title} eyebrow="Metric matrix">
      <div className={styles.metricGrid}>
        {metrics.map((metric) => (
          <article
            key={metric.label}
            className={`${styles.metricTile} ${styles[`family-${metric.family}`]}`}
          >
            ...
            <p className={styles.metricLever}>
              <span>First lever:</span> {metric.lever}
            </p>
          </article>
        ))}
      </div>
      <table className={styles.dataTable}>
        ...
        <thead>
          <tr>
            ...
            <th scope="col">First investigation</th>
          </tr>
        </thead>
        <tbody>
          {metrics.map((metric) => (
            <tr key={`${metric.label}-row`}>
    ...
    <VisualShell id={id} title={title} eyebrow="Worked example">
      <div className={styles.exampleSummary}>
        <div>
          ...
          <strong>{formatPercent(metrics.acos)}</strong>
        </div>
        <div>
          <span>Target ACoS</span>
          <strong>{formatPercent(target)}</strong>
        </div>
        <div className={gap > 0 ? styles.negative : styles.positive}>
      ...
      <div
        ...
        aria-label="Coffee grinder funnel from impressions to ad sales"
      >
      ...
      <table className={styles.dataTable}>
        ...
        <tbody>
          <tr>
            ...
            <td>Cost of each click</td>
          </tr>
          <tr>
            <th scope="row">CTR</th>
            <td>160 ÷ 20,000</td>
            <td>{formatPercent(metrics.ctr)}</td>
            <td>Traffic appeal</td>
          </tr>
          <tr>
            <th scope="row">CVR</th>
            <td>8 ÷ 160</td>
            <td>{formatPercent(metrics.cvr)}</td>
            <td>Listing and intent fit</td>
          </tr>
          <tr>
            <th scope="row">ACoS</th>
            <td>192 ÷ 320</td>
            <td>{formatPercent(metrics.acos)}</td>
            <td>Ad efficiency against target</td>
          </tr>
          <tr>
            <th scope="row">TACoS</th>
            <td>192 ÷ 800</td>
            <td>{formatPercent(metrics.tacos)}</td>
            <td>Store-level ad burden</td>
          </tr>
          <tr>
            <th scope="row">ROAS</th>
            <td>320 ÷ 192</td>
            <td>{metrics.roas.toFixed(2)}x</td>
            <td>Sales returned per peso spent</td>
          </tr>
        </tbody>
      </table>
      <p className={styles.visualTakeaway}>
        ...
        {gap > 0
          ? "cost is above the supplied target, but the first cause still depends on objective, data window, traffic, and conversion evidence."
          : "the result is below the supplied target; confirm the objective and data window before scaling."}
      </p>
    </VisualShell>
  );
}
          ...
          <li key={step.number} className={styles.decisionStep}>
            ...
            <div className={styles.decisionContent}>
              ...
              <p className={styles.decisionAction}>
                <strong>Then:</strong> {step.action}
              </p>
            </div>
          </li>
        ))}
...
function PatternBoard({ id, title, patterns }: { id: string; title: string; patterns: Pattern[] }) {
  return (
    <VisualShell id={id} title={title} eyebrow="Pattern library">
      <div
        className={styles.patternBoard}
        role="region"
        aria-label="Signal to action pattern library"
        tabIndex={0}
      >
        <table className={styles.dataTable}>
          ...
          <thead>
            <tr>
              ...
              <th scope="col">Likely lever</th>
              <th scope="col">Do not do first</th>
            </tr>
          </thead>
          <tbody>
            {patterns.map((pattern) => (
              <tr key={pattern.signal}>
                <th scope="row">{pattern.signal}</th>
                <td>{pattern.firstCheck}</td>
                <td>{pattern.lever}</td>
                <td>{pattern.avoid}</td>
              </tr>
            ))}
      ...
      <div className={styles.practiceGrid}>
        <div className={styles.practiceCase}>
          ...
          <dl className={styles.caseFacts}>
            <div>
              <dt>Impressions</dt>
              <dd>{data.impressions.toLocaleString()}</dd>
            </div>
            <div>
              <dt>Clicks</dt>
            ...
            <div>
              <dt>Orders</dt>
              <dd>{data.orders.toLocaleString()}</dd>
            </div>
            <div>
              <dt>Spend</dt>
              <dd>{formatMoney(data.spend)}</dd>
            </div>
            <div>
              <dt>Ad sales</dt>
              <dd>{formatMoney(data.adSales)}</dd>
            </div>
            <div>
              <dt>Target ACoS</dt>
              <dd>{formatPercent(data.targetAcos ?? 25)}</dd>
            </div>
          </dl>
        </div>
        <div className={styles.practiceAction}>
          ...
          <p>
            Calculate CTR, CVR, and ACoS. Then name the first question you would investigate before
            changing the bid.
          </p>
    ...
    <VisualShell id={id} title={title} eyebrow="Client language">
      <div className={styles.recommendation}>
        {parts.map((part, index) => (
          <span key={`${part}-${index}`} className={styles.recommendationPart}>
            <b>{index + 1}</b>
            {part}
          </span>
        ))}
      ...
      <p className={styles.visualTakeaway}>
        A defensible recommendation names the target, pattern, next check, action boundary, and
        review date.
      </p>
    </VisualShell>
  );
...
export function VisualLessonBlock({ id, kind, title, body }: VisualLessonBlockProps) {
  ...
  if (kind === "metric-matrix")
    return <MetricMatrix id={id} title={title} metrics={data.metrics ?? []} />;
  ...
  if (kind === "decision-tree")
    return <DecisionTree id={id} title={title} steps={data.steps ?? []} />;
  if (kind === "pattern-board")
    return <PatternBoard id={id} title={title} patterns={data.patterns ?? []} />;
  if (kind === "practice-workbench")
    return (
      <PracticeWorkbench
        id={id}
        title={title}
        ...
        prompt={data.prompt ?? "Work the case."}
        answer={
          data.answer ?? "State the objective, data window, evidence, next check, and review date."
        }
      />
    );
  if (kind === "recommendation-builder")
    return (
      <RecommendationBuilder
        id={id}
        title={title}
        sentence={data.sentence ?? "|Target|Pattern|Next check|Review date|"}
      />
    );
  return null;
}
