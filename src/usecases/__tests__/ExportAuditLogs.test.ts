...
import type { IAuditLog, AuditLogPage } from "@/ports/repositories/IAuditLog";
...
describe("ExportAuditLogs", () => {
  it("returns all entries collected from a single page", async () => {
    ...
    const list = vi.fn<[], Promise<ReturnType<typeof Result.ok<AuditLogPage>>>>(
      async () => Result.ok(mockPage) as never,
    );
  ...
  it("collects entries from multiple pages until nextCursor is null", async () => {
    ...
    let callCount = 0;
    const list = vi.fn<[], Promise<ReturnType<typeof Result.ok<AuditLogPage>>>>(async () => {
      callCount++;
      ...
      return Result.ok(pages[callCount - 1] ?? page2) as never;
    });
    const repo = { list } as unknown as IAuditLog;
    const useCase = new ExportAuditLogs({ auditLog: repo });
  ...
  it("propagates db_error when repo list fails", async () => {
    const list = vi.fn<
      [],
      Promise<ReturnType<typeof Result.err<{ kind: "db_error"; message: string }>>>
    >(async () => Result.err({ kind: "db_error", message: "connection lost" }) as never);
    const repo = { list } as unknown as IAuditLog;
    const useCase = new ExportAuditLogs({ auditLog: repo });
  ...
  it("returns empty entries when repo has no data", async () => {
    ...
    const list = vi.fn<[], Promise<ReturnType<typeof Result.ok<AuditLogPage>>>>(
      async () => Result.ok(emptyPage) as never,
    );
    const repo = { list } as unknown as IAuditLog;
    const useCase = new ExportAuditLogs({ auditLog: repo });
