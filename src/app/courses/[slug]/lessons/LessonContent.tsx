...
function renderAmphDiv(props: AmphBlockProps): ReactElement | null {
  ...
  if (block === "comparison-table") {
    ...
    return (
      <ComparisonTable
        id={props["data-amph-id"] ?? "comparison-table"}
        title={props["data-amph-title"] ?? "Comparison"}
  ...
  if (block === "formula-ladder") {
    ...
    return (
      <FormulaLadder
        id={props["data-amph-id"] ?? "formula-ladder"}
        title={props["data-amph-title"] ?? "Formula"}
        steps={data.steps}
        result={data.result}
        note={data.note}
      />
  ...
  if (block === "classification-board") {
    const data = parseAmphJson<{
      ...
      items?: { id: string; label: string; categoryId: string; rationale?: string }[];
      prompt?: string;
    }>(jsonBody);
    if (!data?.categories || !data.items) return null;
    return (
      <ClassificationBoard
        id={props["data-amph-id"] ?? "classification-board"}
        title={props["data-amph-title"] ?? "Classification"}
        categories={data.categories}
        items={data.items}
        prompt={data.prompt}
        revealMode={props["data-amph-reveal-mode"]}
      />
    );
  }
  ...
  if (block === "decision-flow") {
    const data = parseAmphJson<{
      steps?: { id: string; label: string; question: string; evidence: string; action: string }[];
      initialStep?: number;
    }>(jsonBody);
    if (!data?.steps) return null;
    return (
      <DecisionFlow
        id={props["data-amph-id"] ?? "decision-flow"}
        title={props["data-amph-title"] ?? "Decision flow"}
        steps={data.steps}
  ...
  if (block === "simulation-rubric") {
    const data = parseAmphJson<{
      scenario: string;
      criteria: { id: string; label: string; lookFor: string; commonError?: string }[];
      submission?: string;
    }>(jsonBody);
    if (!data?.scenario || !data.criteria) return null;
    return (
      <SimulationRubric
        id={props["data-amph-id"] ?? "simulation-rubric"}
        title={props["data-amph-title"] ?? "Simulation rubric"}
  ...
  if (block === "annotated-listing") {
    const data = parseAmphJson<{
      sections?: { id: string; label: string; role: string; content: string; effect: string }[];
      prompt?: string;
    }>(jsonBody);
    if (!data?.sections) return null;
    return (
      <AnnotatedListingCanvas
        id={props["data-amph-id"] ?? "annotated-listing"}
        title={props["data-amph-title"] ?? "Listing anatomy"}
  ...
  if (block === "hierarchy-builder") {
    ...
    return (
      <HierarchyBuilder
        id={props["data-amph-id"] ?? "hierarchy-builder"}
        title={props["data-amph-title"] ?? "Hierarchy"}
        root={data.root as Parameters<typeof HierarchyBuilder>[0]["root"]}
        note={data.note}
      />
    );
  }
  ...
  if (block === "funnel-canvas") {
    const data = parseAmphJson<{
      stages?: { id: string; label: string; role: string; formats: string[]; question: string }[];
      note?: string;
    }>(jsonBody);
    if (!data?.stages) return null;
    return (
      <FunnelCanvas
        id={props["data-amph-id"] ?? "funnel-canvas"}
        title={props["data-amph-title"] ?? "Full-funnel route"}
        stages={data.stages}
        note={data.note}
      />
    );
  ...
  if (block === "timeline-calendar") {
    ...
    return (
      <TimelineCalendar
        id={props["data-amph-id"] ?? "timeline-calendar"}
        title={props["data-amph-title"] ?? "Timeline"}
  ...
  if (block === "competitive-gap-matrix") {
    ...
    return (
      <CompetitiveGapMatrix
        id={props["data-amph-id"] ?? "competitive-gap-matrix"}
        title={props["data-amph-title"] ?? "Competitive gap"}
  ...
  if (block === "insight-router") {
    ...
    return (
      <InsightRouter
        id={props["data-amph-id"] ?? "insight-router"}
        title={props["data-amph-title"] ?? "Insight router"}
        routes={data.routes}
        revealMode={props["data-amph-reveal-mode"]}
      />
    );
  ...
  if (block === "lesson-pathway") {
    ...
    return (
      <LessonPathway
        id={props["data-amph-id"] ?? "lesson-pathway"}
        title={props["data-amph-title"] ?? "Lesson pathway"}
        steps={data.steps}
        note={data.note}
      />
    );
  ...
  if (block === "simulation-brief") {
    ...
    return (
      <SimulationBriefBuilder
        id={props["data-amph-id"] ?? "simulation-brief"}
        title={props["data-amph-title"] ?? "Simulation brief"}
        fields={data.fields}
        note={data.note}
      />
    );
  ...
  if (block === "portfolio-map") {
    ...
    return (
      <PortfolioMap
        id={props["data-amph-id"] ?? "portfolio-map"}
        title={props["data-amph-title"] ?? "Portfolio map"}
        groups={data.groups}
        note={data.note}
      />
    );
  ...
  if (block === "seasonal-calendar") {
    ...
    return (
      <SeasonalCalendar
        id={props["data-amph-id"] ?? "seasonal-calendar"}
        title={props["data-amph-title"] ?? "Seasonal calendar"}
        phases={data.phases}
        note={data.note}
      />
    );
  ...
  if (block === "evidence-ledger") {
    ...
    return (
      <EvidenceLedger
        id={props["data-amph-id"] ?? "evidence-ledger"}
        title={props["data-amph-title"] ?? "Evidence ledger"}
  ...
  if (block === "sov-positioner") {
    const data = parseAmphJson<{
      bands?: { id: string; label: string; range: string; posture: string; actions: string[] }[];
      note?: string;
    }>(jsonBody);
    if (!data?.bands) return null;
    return (
      <SovPositioner
        id={props["data-amph-id"] ?? "sov-positioner"}
        title={props["data-amph-title"] ?? "SOV positioner"}
        bands={data.bands}
        note={data.note}
      />
    );
