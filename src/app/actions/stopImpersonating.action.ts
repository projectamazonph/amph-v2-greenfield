 ...
 *  4. Return success
 *
 * If there's no backup cookie (should not happen with the current
 * first-time impersonation fix, but kept as a defensive fallback),
 * we fall back to `clearAuthCookie()` so the user is signed out
 * instead of stuck impersonating.
 *
