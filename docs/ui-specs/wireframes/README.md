...
Low-fidelity layout sketches for every planned AMPH screen, one file per screen, styled with the Field Manual tokens in [`assets/tokens.css`](./assets/tokens.css) (same palette as `docs/ui-specs/DESIGN-SPEC.md` §1-2). Open any `.html` file directly in a browser — no build step.
