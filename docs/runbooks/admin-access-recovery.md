...
- There is no admin-specific login route — an admin signs in through the normal `/login` flow, and `requireAdmin()` (`src/lib/auth.ts:143-149`) just checks `user.role === "ADMIN"` on the resulting session.
- There is **no admin 2FA** (confirmed absent, see `docs/audit-2026-07-26-hardening-review.md`).
