...
│               │   └── edit/page.tsx
│               └── new/page.tsx
├── payments/                         # Order-based; no separate Payment table
│   ├── page.tsx
│   └── [id]/
...
| Revenue (24h)           | sum `Order.amount` where `status = COMPLETED` and `paidAt > now - 24h`                                                   |
| Revenue (7d)            | same, 7d                                                                                                                 |
| Revenue (30d)           | same, 30d                                                                                                                |
| Active enrollments      | `Enrollment.count({ status = ACTIVE })`                                                                                  |
| Refund rate (30d)       | `Order.count({ status = REFUNDED, refundedAt > now - 30d })` / `Order.count({ status = COMPLETED, paidAt > now - 30d })` |
...
Quiz management lives at `/admin/quizzes` (STORY-091), NOT inside the module editor. The earlier "JSON blob nested in the module editor" design was never built and is superseded by the dedicated surface below.
...
### Simulator detail (`/admin/simulators/[id]`)
...
### Scenario editor (`/admin/simulators/[id]/edit`)
...
Form: refundWindowDays, earlyBirdLimit, earlyBirdPriceMinor, featureFlags (JSON editor). All fields are admin-only.

> **Note:** Business profile fields (businessName, businessTin, businessAddress) and BIR receipt settings are planned (Sprint 13). The `BusinessProfile` table does not yet exist. `SettingsRepository` maps to the existing `PricingTier` and feature flag config — not a `BusinessProfile` entity.
