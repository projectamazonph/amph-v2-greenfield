...
**Current review:** 2026-08-12 against `ee1737a`. JWTs carrying a `sessionId` are checked against the session repository, and login enforces `lockedUntil`. Historical findings below should be read in their dated context.
...
3. **Rate limits are now wired into auth use cases.** The `RateLimiter` port and `UpstashRateLimiter` / `InMemoryRateLimiter` adapters are in place, and the auth use cases that have a documented rate-limit window now call `rateLimiter.check()` directly: `RequestPasswordReset` (5/hr per email, 20/hr per IP), `ResendVerification` (1 per 60s per user). The action-layer wrappers (`signUpAction`, `loginAction`) don't add an extra check because the use cases they invoke already enforce the limits. Update this issue as resolved if the action-layer concern is covered by the use-case-level enforcement.
