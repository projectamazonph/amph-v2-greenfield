...
This produces 13 modules and 45 lessons (Module -1, the Amazon primer, was added after the original sync). Module and lesson slugs remain stable for existing content. New lessons use the source repository’s `m9l1` through `m11l4` sequence with greenfield-compatible filenames and frontmatter.
