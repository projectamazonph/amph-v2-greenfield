...
| Course prerequisites         | Implemented   | Admins set course or single-lesson gates at `/admin/courses/[id]/prerequisites` (audited). `EnrollStudent` blocks gated enrollment with `prerequisite_not_met` for every entitlement except `admin_grant`; the course page names the required course before purchase (P1-01).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Assignments                  | Implemented   | Admins assign work at `/admin/assignments` (search, filter, pagination, audited create/grade). Students see their queue at `/assignments` with due dates, overdue highlight, grades, and feedback, and submit with one button. Lifecycle is PENDING to SUBMITTED to GRADED with a 0-100 integer grade; overdue is derived, never persisted (P1-02).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| OAuth social login           | Implemented   | Google sign-in with verified-email auto-signup, account linking, guarded unlinking, and TOTP-bypass refusal; session issuance mirrors password login. Env-gated: no credentials means no button and fallback routes (P1-04).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Admin settings               | Implemented   | TOTP plus an admin-editable site settings store (`/admin/settings`, audited). The 503 maintenance page reads `support_email` best-effort with a hardcoded fallback (P1-05).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Learning-experience uplift   | Partial       | Wave 0 has a checked-in curriculum inventory (42 lessons, 443 planned minutes), durable planned learner time, and a public-claim contract (`STORY-111`–`STORY-113`, `STORY-129`). Wave 1 closes the zero-to-one onboarding arc: optional pre-course diagnostic, glossary tooltips, guided first-decision route, and onboarding completion view (`STORY-130`–`STORY-133`). Wave 3 adds a typed lesson-to-tool bridge validator chained into `validate:learning-release` so a broken tool route, tier mismatch, or unpublished scenario fails the release check before the production promotion (`STORY-134`). The lesson-production contract and content passes now cover Modules 0 through 5, including independent calculations, keyword grouping decisions, listing-audit rationales, campaign pre-flight maps, and weekly client readouts (`STORY-117`–`STORY-121`, `STORY-127`, `STORY-128`). Evidence-linked tool practice and the rubric-backed capstone remain in the build plan. Existing simulator scores remain formative. |
...
- Full Vitest run: 4,000 passing, 3 skipped.
