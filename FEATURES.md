...
| Transactional email          | Implemented          | All nine transactional scenarios use a shared polished HTML template system. `/admin/email-templates` personalizes the seven eligible send paths with type-specific `{{variables}}`; invalid tokens are rejected on save, refunds link safely to the student dashboard, and the failed-checkout flow sends the payment-failed retry email once to its authenticated buyer. |
